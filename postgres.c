...
exec_simple_query(const char *query_string)
{
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		/*
		 * PROJECT CODE
		 *  1. Insertions Handling
		 *  2. Updates Handling
		 */

		/* Extracting the command from the query */
		 ...
		 int i=0,j=0;
		 while(i < query_len && query_str[i] == ' ')
		 {
			++i;
		 }
		 while(i < query_len && query_str[i] != ' ')
		 {
			char ch = (char)tolower(query_str[i++]);
			query_cmd[j++] = ch;
		 }
		 query_cmd[j] = '\0';
		 ...
		 /* 1. Insertion handling code */
		 if(strcmp("insert",query_cmd) == 0)
		 {

			check_insert_fd_violation(query_str,query_len,i);

		 }
		 else if(strcmp("update",query_cmd) == 0)
		 {
			check_update_fd_violation(query_str,query_len,i);
		 }
